...
function clientSubscription( productFamily, capability, someToken ) {
  debug( 'clientSubscription', productFamily, capability, someToken );
...
function onceMounted( parent ) {
  ...
  //console.log( require( 'util' ).inspect( module.package, { showHidden: false, depth: 2, colors: true } ) );

  if( parent.get( 'stripeKeys' ) ) {
    app.set( 'stripeKeys', parent.get( 'stripeKeys' )  );
  } else {
  ...
  if( _.get( customers, 'data' ) ) {
    ...
    _.each(_.get( customers, 'data' ), function( customer ) {
      ...
      _.each(_.get( customer, 'subscriptions.data' ), function eachSubscription( subscription ) {
        ...
        var _customer = _.defaultsDeep( { id: undefined, stripeKey: stripeSetup.stripeKey, productFamily: stripeSetup.productFamily}, subscription, { metadata: customer.metadata, customerData: customer });

        _customer.metadata = dot.object( _customer.metadata );
